...
# DuckDB resource limits. The development laptop has 7.7 GB RAM with little free; DuckDB
# spills to the temp directory beyond this.
...
def _to_parquet(con: duckdb.DuckDBPyConnection, csv: Path, parquet: Path) -> None:
    if parquet.exists() and parquet.stat().st_mtime >= csv.stat().st_mtime:
        log.info("reusing %s", parquet.name)
        return
...
def connect(raw: Path, processed: Path) -> duckdb.DuckDBPyConnection:
    ...
    tx_parquet = processed / "transactions.parquet"
    id_parquet = processed / "identity.parquet"
    _to_parquet(con, raw / "transactions.csv", tx_parquet)
    ...
    con.execute(
        ...
        f"FROM read_parquet('{id_parquet.as_posix()}')"
    )
    for view, name in (("cc", "closed_cases_history.csv"), ("cp", "case_pack.csv")):
        con.execute(
...
def main() -> None:
    ...
    con = connect(args.raw, args.processed)
    report = Report(con)
    for section in (
        section_shape,
        section_ids,
